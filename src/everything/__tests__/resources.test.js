...
import { textResource, blobResource, textResourceUri, blobResourceUri, RESOURCE_TYPE_TEXT, RESOURCE_TYPE_BLOB, RESOURCE_TYPES, resourceTypeCompleter, resourceIdForPromptCompleter, resourceIdForResourceTemplateCompleter, registerResourceTemplates, } from '../resources/templates.js';
...
import { setSubscriptionHandlers, beginSimulatedResourceUpdates, stopSimulatedResourceUpdates, } from '../resources/subscriptions.js';
describe('Resource Templates', () => {
    ...
    describe('registerResourceTemplates', () => {
        it('should register text and blob resource templates', () => {
            ...
            const textRegistration = registeredResources.find((r) => r[0].includes('Text'));
            ...
            const blobRegistration = registeredResources.find((r) => r[0].includes('Blob'));
...
describe('Session Resources', () => {
    describe('getSessionResourceURI', () => {
        ...
        it('should handle various resource names', () => {
            ...
            expect(getSessionResourceURI('document_123')).toBe('demo://resource/session/document_123');
        });
    });
    describe('registerSessionResource', () => {
        it('should register text resource and return resource link', () => {
            ...
            const result = registerSessionResource(mockServer, resource, 'text', 'Hello, World!');
            expect(result.type).toBe('resource_link');
            expect(result.uri).toBe(resource.uri);
            expect(result.name).toBe(resource.name);
            expect(mockServer.registerResource).toHaveBeenCalledWith('test-file', 'demo://resource/session/test-file', expect.objectContaining({
                mimeType: 'text/plain',
                description: 'A test file',
            }), expect.any(Function));
        });
