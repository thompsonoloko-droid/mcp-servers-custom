...
function createMockServer() {
    ...
    const mockServer = {
        registerPrompt: vi.fn((name, config, handler) => {
...
describe('Prompts', () => {
    ...
    describe('args-prompt', () => {
        ...
        it('should include city and state in message', () => {
            ...
            expect(result.messages[0].content.text).toBe("What's weather in San Francisco, California?");
        });
    ...
    describe('completable-prompt', () => {
        it('should generate promotion message with department and name', () => {
            ...
            expect(result.messages[0].content.text).toBe('Please promote Alice to the head of the Engineering team.');
        });
    ...
    describe('resource-prompt', () => {
        ...
        it('should reject invalid resource type', () => {
            ...
            registerEmbeddedResourcePrompt(mockServer);
            const handler = handlers.get('resource-prompt');
            expect(() => handler({ resourceType: 'Invalid', resourceId: '1' })).toThrow('Invalid resourceType');
        });
        it('should reject invalid resource ID', () => {
            const { mockServer, handlers } = createMockServer();
            registerEmbeddedResourcePrompt(mockServer);
            const handler = handlers.get('resource-prompt');
            expect(() => handler({ resourceType: 'Text', resourceId: '-1' })).toThrow('Invalid resourceId');
            expect(() => handler({ resourceType: 'Text', resourceId: '0' })).toThrow('Invalid resourceId');
            expect(() => handler({ resourceType: 'Text', resourceId: 'abc' })).toThrow('Invalid resourceId');
        });
